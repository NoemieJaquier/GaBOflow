...
from BoManifolds.Riemannian_utils.sphere_utils import get_axisangle, expmap
...
def plot_sphere(ax, base=[0, 0, 1], color=[0.8, 0.8, 0.8], alpha=0.8, r=0.99, linewidth=0, lim=1.1, n_elems=100,
                **kwargs):
    """
    Plot a sphere
    ...
    Optional parameters
    -------------------
    :param base: base point of the sphere
...
def plot_sphere_tangentplane(ax, base, l_vert=1, color='w', alpha=0.15, linewidth=0.5, **kwargs):
    """
    Plot tangent plane of a point lying on the manifold
    Based on the function of riepybdlib (https://gitlab.martijnzeestraten.nl/martijn/riepybdlib)

    Parameters
    ----------
    :param ax: figure aces
...
def plot_gaussian_on_sphere_tangent_space(ax, mu, sigma, color='red', linewidth=2, linealpha=1, planealpha=0.2,
                                          label='', showtangent=True, **kwargs):
    """
    Plot (mean and) covariance in the manifold
    Based on the function of riepybdlib (https://gitlab.martijnzeestraten.nl/martijn/riepybdlib)
